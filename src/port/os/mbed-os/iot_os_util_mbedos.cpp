		...
		const int wait_for_all_bits, const unsigned int wait_time_ms)
{
	...
	if (wait_for_all_bits) {
		...
		if (ret & osFlagsError) {
			IOT_ERROR("Event not received for bits 0x%x | Handle: %p [0x%x]",
	...
	if (ret & osFlagsError) {
		IOT_ERROR("Did not receive Event for bits 0x%x | Handle: %p [0x%x]", bits_to_wait_for,
				eventgroup_handle, ret);
		return 0;
	}
	IOT_INFO("Received ANY | Handle: %p | Bits: 0x%x | Value: 0x%x\n",
			eventgroup_handle, bits_to_wait_for, ret);
	return ret;
}
